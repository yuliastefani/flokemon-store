...
(deffunction addFlokemonFire()
    ...
    (bind ?fireDfs 0) 
    (while (or (< ?fireDfs 5) (> ?fireDfs 300))
...
(deffunction addFlokemonWater()
    ...
    (bind ?waterDfs 0) 
    (while (or (< ?waterDfs 5) (> ?waterDfs 300))
        (printout t "Insert Flokemon Defense [5 - 300] : ")
        (bind ?waterDfs (read))
    )
...
(deffunction updateFlokemonFire()
    ...
    (bind ?idx 0)
    (while (or (< ?idx 1) (> ?idx ?*numFlokemonFire*))
...
(while (neq ?choose 6)
    ...
    ; Masuk menu REMOVE  
        ...
        (while (or (< ?choose3 1) (> ?choose3 3))
            ...
            (if (eq ?choose3 1)
                ...
                (viewFlokemonWater)
                (bind ?idx 0)
                (while (or (< ?idx 1) (> ?idx ?*numFlokemonWater*))
    ...
    ; Masuk menu FIND
        ...
        then 
        (bind ?type "")
        (while (and (neq (str-compare ?type "Fire")0) (neq (str-compare ?type "Water")0))
            (printout t "Demanded type [Fire | Water]:")
        	(bind ?type (read)))            
            
    	(bind ?power "")
        (while (and (neq (str-compare ?power "Weak")0) (neq (str-compare ?power "Strong")0))
            (printout t "Demanded power [Weak | Strong]:")
        	(bind ?power (read)))            
            
		(bind ?defense "")
        (while (and (neq (str-compare ?defense "Soft")0) (neq (str-compare ?defense "Hard")0))
            (printout t "Demanded defense [Soft | Hard]:")
        	(bind ?defense (read)))
        
        (bind ?lvl 0)
        (while (or (< ?lvl 1) (> ?lvl 100))
            (printout t "Demanded minimum flokemon level [1 - 100]:")
        	(bind ?lvl (read)))            
        
        (bind ?budget 0)
        (while (or (< ?budget 1000) (> ?budget 1000000))
            (printout t "Budget for flokemon [1000 - 1000000]:")
        	(bind ?budget (read)))    
        
        (bind ?confirm "")
        (while (and (neq (str-compare ?confirm "Y")0) (neq (str-compare ?confirm "N")0))
